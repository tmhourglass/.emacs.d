...
;; 这个包用于测试加载启动的执行时间，用法如下： (可以不关闭当前Emacs进行测试)
;; emacs --init-directory=~/config/tmhourglass-emacs -Q -l ~/config/tmhourglass-emacs/profile-dotemacs.el -f profile-dotemacs
;; 说明：先在该文件中配置profile-dotemacs-file，即需要测试的文件。再执行上面命令，即可显示执行情况及占比
;; --init-directory指定配置目录，-l 指定加载的el文件，-f 指定需要执行的函数（即该el文件中的函数，即profile-dotemacs）
;; 不再需要chemacs2，直接执行--init-directory即可使用不同配置，另外其他的参数，可通过emacs --help 查看到
